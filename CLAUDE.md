...
- **Released: v1.0.0 (2026-06-17) — Phases 0–7 complete.** Full acceptance F1–F12 met (`docs/phase-7-acceptance-audit.md`); schema **frozen at v1.0** (ADR-009; `specVersion ^1.\d+$`). 11 families (flowchart, architecture/C4, dependency, swimlane, sequence, ER, state, deployment, class, mindmap, **activity**); SVG/PNG/PDF + editable draw.io/PPTX writers; **agent surface** (`tarseem.generate`, `schema_bundle()`, CLI `generate`/`schema`/`migrate`, reference skill at `integrations/claude-skill/SKILL.md`); diagram types are entry-point plugins (ADR-008, dogfooded by the built-ins + two external example plugins). Deferred: Mermaid/PlantUML source writers, searchable-Arabic PDF, the mkdocs site + generated per-object schema reference. Per-phase progress in `docs/spikes/`.
